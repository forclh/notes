...
def paginated_query(total_items, page_size):
    ...
    # 你的代码
    current = 0
    while (current + page_size) < total_items :
        yield total_items[current: current + page_size]
        current += page_size
    yield total_items[current:]
