...
| 20   | [[python/m8duna\|20-类型标注]]                                   | 变量/函数标注、`|` 联合类型、容器类型、`type` 别名、`Self`、泛型 `[T]`、`Callable`、`TypedDict`、`type: ignore` |
| 21   | [[python/2g915n\|21-模块化]]                                     | 模块/包/成员、`__all__`、`__init__.py`、相对导入、搜索路径、循环导入                            |
| 22   | [[python/u1r8ob\|22-标准库]]                                     | 官方文档导航、树形目录展示、Markdown 合并作业                                                    |
| 23   | [[python/jirzuw\|23-第三方库]]                                   | pip、镜像源、版本约束、requirements、venv、AI 聊天工具实战                                       |
| 24   | [[python/qooqqt\|24-事件循环]]                                   | 同步 vs 异步、事件循环、`run_forever`、`call_soon`/`call_later`、队列调度预测                    |
